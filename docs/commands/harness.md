...
`/docs/` 하위 문서(PRD, ARCHITECTURE, ADR 등)를 읽고 프로젝트의 기획·아키텍처·설계 의도를 파악한다. 필요시 Gemini의 탐색 기능을 활용한다.
...
1. `/docs/tests/` 하위의 관련 테스트 명세를 먼저 읽어라.
2. 명세에 정의된 시나리오에 따라 **실패하는 테스트를 먼저 작성**하라.
3. 작성한 테스트가 실패하는 것을 확인한 뒤, 이를 통과시키기 위한 실제 로직을 구현하라. (Green phase)
4. 필요한 경우 코드를 리팩토링하라.
